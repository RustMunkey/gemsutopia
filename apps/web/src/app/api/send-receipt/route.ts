...
interface OrderData {
  ...
  items: OrderItem[];
  subtotal: number;
  tax?: number; // Deprecated - tax included in product price
  shipping: number;
  total: number;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      return ApiError.externalService('Resend', 'Email service not configured - missing API key');
    }
    ...
    const fromEmail = process.env.RESEND_FROM_EMAIL || 'noreply@gemsutopia.ca';

    // Send to customer
    const customerResult = await resend.emails.send({
      from: `Gemsutopia <${fromEmail}>`,
    ...
    const adminResult = await resend.emails.send({
      from: `Gemsutopia <${fromEmail}>`,
      to: [process.env.ADMIN_NOTIFICATION_EMAIL || 'gemsutopia@gmail.com'],
    ...
    return apiSuccess(
      ...
      'Receipt sent to both customer and admin'
    );
  } catch (error) {
