...
export const POST = requireAdmin(async (request: NextRequest) => {
  try {
    ...
    if (!validation.isValid) {
      return ApiError.validation('Invalid input data', { errors: validation.errors });
    }
    ...
    // Shipping settings
    if (updates.enableShipping !== undefined)
      dbUpdates.enable_shipping = updates.enableShipping.toString();
    if (updates.internationalShipping !== undefined)
      dbUpdates.international_shipping = updates.internationalShipping.toString();
    if (updates.singleItemShippingCAD !== undefined)
      dbUpdates.single_item_shipping_cad = updates.singleItemShippingCAD.toString();
    if (updates.singleItemShippingUSD !== undefined)
      dbUpdates.single_item_shipping_usd = updates.singleItemShippingUSD.toString();
    if (updates.combinedShippingCAD !== undefined)
      dbUpdates.combined_shipping_cad = updates.combinedShippingCAD.toString();
    if (updates.combinedShippingUSD !== undefined)
      dbUpdates.combined_shipping_usd = updates.combinedShippingUSD.toString();
    if (updates.combinedShippingEnabled !== undefined)
      dbUpdates.combined_shipping_enabled = updates.combinedShippingEnabled.toString();
    if (updates.combinedShippingThreshold !== undefined)
      dbUpdates.combined_shipping_threshold = updates.combinedShippingThreshold.toString();
    ...
    if (updates.openGraphDescription !== undefined)
      dbUpdates.open_graph_description = updates.openGraphDescription;
    ...
    if (updates.twitterDescription !== undefined)
      dbUpdates.twitter_description = updates.twitterDescription;
    ...
    if (Object.keys(dbUpdates).length > 0) {
      ...
      if (!dbSuccess) {
        return ApiError.database('Failed to save settings to database');
      }
    }
    ...
    return apiSuccess({ settings: updatedSettings }, 'Settings updated successfully');
  } catch {
    return ApiError.internal();
  }
});
