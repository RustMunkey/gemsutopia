-- SUPABASE QUERY OPTIMIZATIONS (FIXED - NO CONCURRENTLY)
-- Run these in your Supabase SQL Editor

-- 1. Create performance indexes (without CONCURRENTLY)
DO $$
BEGIN
    ...
    IF EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'auctions' AND column_name = 'status') THEN
        CREATE INDEX IF NOT EXISTS idx_auctions_status_active
    ...
    IF EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'auctions' AND column_name = 'current_bid') THEN
        CREATE INDEX IF NOT EXISTS idx_auctions_current_bid
